                ...
                try:
                    if ident is not None:
                        ...
                        # for taskset in tasksets:
                            freeze_support()
                            p = Pool(5)
                            rel = (p.map(func_star, itertools.izip(tasksets,itertools.repeat(max_fault_rate))))
                        print rel
                        for i in rel:
                            results_chernoff.append(i[0])
                            results_george.append(i[1])
                        np.save('../results_short/mp_res_chernoff_' + filename + '.npy', results_chernoff)
                        np.save('../results_short/mp_res_george_' + filename + '.npy', results_george)
                    else:
                        raise Exception, "Please specify an identifier!"
                except IOError:
                    print 'Could not write filename %s' % filename
...
def insideroutine(taskset, max_fault_rate):
    ...
    results_george.append({'ErrProb' : 1, 'ms' : 1})
    print 'DONE!'
    return [results_chernoff, results_george]
